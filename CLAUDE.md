...
- **Phase 0–2 工程完成、已上线、全部质量闸门绿灯。**
- 已配准 **4 张**历史图：`chengdu-1911` / `chengdu-1915`（左右拼幅）/ `chengdu-1933`（首屏默认）/ `chengdu-1944`。
- `MapViewer` 已**泛化**为「遍历 `epochs`/`maps` 登记表 + 时间轴驱动」：**新增一张图无需改渲染代码**（流程见 [`docs/adding-a-map.md`](./docs/adding-a-map.md)）。
- **下一步功能性工作 = Phase 3**（卷帘对照 / URL 深链接 / 移动端）；Phase 4 = 标注层（架构已按 plan §5 预留）。
...
**提交前五道闸门须全绿**：`pnpm check && pnpm lint && pnpm format:check && pnpm validate && pnpm test`（与 CI `.github/workflows/ci.yml` 一致）。
...
- `src/lib/` — **纯逻辑，单元测试主战场**：`timeline.ts`（epoch 选择/排序/步进）、`annotations.ts`（`import.meta.glob` 自动装载标注）、`opacity.ts`。
