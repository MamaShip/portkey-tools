...
> **未来（架构已预留，尚未落地）**：`src/data/annotations-features.geojson`（Phase 4 标注层，见 §5.3）、`e2e/`（Phase 3 Playwright 冒烟，ci.yml 中任务已占位）。
...
| **2 时间轴+全部图+合规** ✅（工程完成；增图为持续内容工作） | 配准其余图并登记（现 4 张：1911 / 1915 拼幅 / 1933 / 1944）；离散时间轴 + epoch 切换（含 `现今`）；来源/版权（落地为页内 `SourcesModal` 弹窗）+ 举报 `mailto:` | 已上线 `tools.portkey.click/cd-old-map`：时间轴切换逐层换图；弹窗列出各图出处 | **全部**图的 schema + 配准 sanity 校验；**引用完整性测试**：epochs 引用的每个 `mapId` 在 maps 表存在、每个标注文件存在；epoch 排序/选择逻辑单元测试 |
...
> **当前进度（2026-06）：Phase 0–2 工程完成、已上线。** `tools.portkey.click/cd-old-map` 上有 4 张配准历史图（1911 / 1915 拼幅 / 1933 / 1944）、离散时间轴、来源/版权弹窗，全部质量闸门绿灯。`MapViewer` 已泛化为「遍历登记表 + 时间轴驱动」，新增一张图无需改渲染代码（流程见 `docs/adding-a-map.md`）。**继续向 5–8 张目标增图（尤其 1950s–1975 时段）属架构已支持的持续内容工作，只待源扫描图。** 下一步功能性工作是 Phase 3。上手与架构详解见 `docs/development.md`，状态速览见 `CLAUDE.md`。
