...
- **卷帘/swipe 对照**（增强项，后续）：在现代图与某历史图之间做左右滑动对比，可用 MapLibre 的 `maplibre-gl-compare` 模式（双图同步）或对叠加层 canvas 做 `clip-path`。
- **可分享的深链接**（后续）：把 `epoch + 经纬度 + zoom` 序列化进 URL hash，便于分享"某条街 1949 vs 现今"。状态设计时即考虑可序列化。

### 4.5 站点框架与工程

**Astro + TypeScript（islands 架构）**：网站整体静态优先、默认不发 JS；地图工具作为单个 **React island** 挂在 `/tools/chengdu-historical-map` 路由，其余页面（首页、工具索引、关于、版权说明）保持极轻。这天然支持"网站随时间长出更多工具"——每个新工具就是一个新路由。TypeScript 全程使用（学习 + 类型安全 + 更利于 AI 协作）。
...
/
├─ .github/workflows/
│  ├─ ci.yml                              # 类型检查/lint/数据校验/测试/构建（随附）
│  └─ sync-tiles.yml                      # 手动触发：把 tiles/ 同步到 Wasabi（可选）
├─ src/
│  ├─ pages/
│  │  ├─ index.astro                      # 首页 / 工具索引
│  │  ├─ about.astro
│  │  ├─ copyright.astro                  # 来源与版权 + 侵权举报入口
│  │  └─ tools/
│  │     └─ chengdu-historical-map.astro  # 挂载地图 island 的路由
│  ├─ components/
│  │  ├─ MapViewer.tsx                     # React island：MapLibre + 图层 + 状态
│  │  ├─ Timeline.tsx                      # 离散时间轴
│  │  └─ OpacityControl.tsx
│  ├─ data/
│  │  ├─ epochs.ts
│  │  ├─ maps.ts
│  │  ├─ schema.ts                         # zod schema + 引用完整性校验
│  │  ├─ annotations/                      # 各图的 Allmaps 配准标注 JSON
│  │  └─ annotations-features.geojson      # 标注要素（v2）
│  ├─ lib/                                 # 纯逻辑（epoch 选择、时间过滤、URL hash 序列化）—— 单元测试主战场
│  └─ styles/
├─ tests/                                  # Vitest 单元测试 + 配准 sanity 测试
├─ e2e/                                    # Playwright DOM 级冒烟
├─ scripts/                                # iiif-tiler 切片、上传 Wasabi 等脚本
├─ public/                                 # 轻量静态资源（图标、样式 JSON 等）
├─ tiles/                                  # 本地瓦片产物（.gitignore，不入库）
├─ LICENSE  README.md  CONTRIBUTING.md  .gitignore
├─ astro.config.mjs
└─ plan.md
```
> 瓦片（IIIF 瓦片、可选 PMTiles）存于 **Wasabi**，**不进 git**；仓库里只存 URL 与几 KB 的标注 JSON。`tiles/` 为本地中转目录，由脚本生成并同步到桶。
