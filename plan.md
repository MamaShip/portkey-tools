...
- **当前定位**（**已落地**，Phase 3）：**复用 MapLibre 自带 `GeolocateControl`**（零新依赖、与右上角 `NavigationControl` 风格一致），而非自写按钮/标记/定位逻辑——这是「尽量简单、不增复杂度」的最优解。可选语义靠控件天性满足：**点击才**请求浏览器定位（不点零授权、零标记）；蓝点+精度圈是 HTML `Marker`，天然浮在老图栅格之上（最上层）；`trackUserLocation` 持续跟踪（移动端「边走边对照」），`enableHighAccuracy` 优先 GPS、无则浏览器回退 WiFi/IP 粗定位。定位点超出成都 `maxBounds` 时由 `outofmaxbounds` 事件触发轻提示（`MapNotice`），而非默默无反应。
