...
> 状态：早期开发（Phase 0–1）。架构与设计决策见 [`plan.md`](./plan.md)；逐步落地清单见 [`phase-0-1-guide.md`](./phase-0-1-guide.md)。
...
完整步骤见 [`phase-0-1-guide.md`](./phase-0-1-guide.md)。概要：

1. 预处理扫描图（裁切、纠斜）。
2. 切静态 IIIF 瓦片：`java -jar iiif-tiler.jar <map>.jpg -version 3 -output ./tiles`。
3. 上传瓦片到对象存储（`rclone copy ...`）。
...
- **代码：[AGPL-3.0-or-later](./LICENSE)（GNU Affero GPL v3）。** 这是**强 copyleft**，并且——因为本项目是一个**网站/网络服务**——选用 AGPL 而非普通 GPL，是为了堵上 GPL 的“SaaS 漏洞”：任何人若修改本代码并**以网络服务形式对外提供**（哪怕从不“分发”二进制），也**必须向其用户提供对应源码**。普通 GPL 不要求这一点。AGPL-3.0 与本项目所用的 MIT/BSD 依赖（Astro、React、MapLibre 等）兼容。
