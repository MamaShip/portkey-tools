...
`order` 越大越新；`mapId` 必须指向 6.1 的 `id`（`pnpm validate` 会查引用完整性）。

### 6.3 ⚠️ 当前渲染器只挂一张图（Phase 1 现状）

[`MapViewer.tsx`](../src/components/MapViewer.tsx) 目前**硬编码加载 `chengdu-1933`**（直接 `import` 标注 + 单个 `WarpedMapLayer`）。因此**今天新增第 2 张图，除了 6.1/6.2 登记，还要改 `MapViewer`**：换/加 `import` 的标注、`maps.find` 的 id、`WarpedMapLayer` 的 `layerId`。

> **Phase 2** 会把 `MapViewer` 泛化为「遍历 `epochs`/`maps` 登记表 + 离散时间轴」，届时新增图就只剩 6.1/6.2 登记 + 配准，不再动渲染代码。在那之前，多图渲染请参照 `MapViewer` 现有写法复制一层。
...
- **⚠️ 配准 sanity 测试** [`tests/georef.test.ts`](../tests/georef.test.ts) 目前也针对 `chengdu-1933`。新增图时给新标注加一段同样的断言（控制点 ≥6、`geo` 落在成都经纬度框 lng 103.6–104.4 / lat 30.3–30.9），或在 Phase 2 泛化为遍历所有标注。
...
src/data/maps.ts
src/data/epochs.ts
src/components/MapViewer.tsx      # 仅 Phase 2 泛化前、手动多挂一张图时
```
