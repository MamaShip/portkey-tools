...
> ⚠️ **本文件是历史竣工记录（Phase 0–1），不是当前上手指南。** 想跑起来或理解架构请看
> **[`docs/development.md`](../development.md)**；项目整体状态速览看 **[`CLAUDE.md`](../../CLAUDE.md)**；
> 新增一张图看 **[`docs/adding-a-map.md`](../adding-a-map.md)**。Phase 2 起项目已演进（详见 `CLAUDE.md`），
> 下文部分细节已被取代——最显著的是**地图页路由由 `/tools/chengdu-historical-map` 精简为 `/cd-old-map`**
> （旧路径经 `public/_redirects` 做 301 跳转）。本文保留为「当时怎么落地」的记录，命令不再逐条更新。
...
> 下方代码为 Phase 0 初始落地记录；当前实现见 [`src/components/MapViewer.tsx`](../../src/components/MapViewer.tsx) 与 **[`docs/basemap.md`](../basemap.md)**。
...
> **[`docs/adding-a-map.md`](../adding-a-map.md)**（SOP，含确切命令与踩坑速查）。下文保留为操作记录。
